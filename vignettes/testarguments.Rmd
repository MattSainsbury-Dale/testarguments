...
The `R` package `testarguments` tests and visualises the performance of a user-defined prediction algorithm over an arbitrary number of arguments and user-defined predictive performance metrics.   

The typical workflow involves:

1. Defining a prediction algorithm, `pred_fun()`, that trains the algorithm and predicts over a testing set. 
2. Defining a set of diagnostics as a function, `diagnostic_fun()`, that quantify the predictive performance given a set of predictions and testing data. 
3. Using `test_arguments()` to train and test `pred_fun()` over a range of argument values. This creates an object of class `testargs`, the central class definition of the package.
4. Visualising the predictive performance with `plot_diagnostics()`. 
